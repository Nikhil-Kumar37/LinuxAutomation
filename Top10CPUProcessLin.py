"""
        ...
        .SYNOPSIS
          To fetch top 10 CPU consuming process.

        .DESCRIPTION
          The script will fetch the top 10 CPU process list.
        ...
        .EXAMPLE
                  > python E:\Linux_Top10CPU_v1.0.py --Server=SERVERNAME --User=USERNAME --Password=PASSWORD  
                           

         
        .NOTES

          Script Name    : Linux_Top10CPU_v1.0.py
          Script Version : 1.0
          Author         : Nikhil Kumar
...
try:                                
        ...
        command1 = "ps -eo pid,comm,%cpu --sort=-%cpu | tr -s ' ' | sed -e 's/^[[:space:]]*//' | grep -v 'PID' | grep -v '/usr/lib/systemd/systemd' | head -10 | tr ' ' ':' | tr '\n' ','"
        ...
        if any("bash" in s for s in error1):
            output = {"retCode" : "1", "retDesc" : "Script Execution Failed", "processlist" : error1}
            write_log(1, "Script Execution Failed")
            print (output)
        else:
            output = {"retCode" : "0", "retDesc" : "Success", "processlist" : output1}
